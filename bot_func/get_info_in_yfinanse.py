...
async def process_excel_file(file_name, chat_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot = context.bot
    try:
        await bot.send_document(chat_id=chat_id, document=open(OUTPUT_FILE, 'rb'))
        await bot.send_message(chat_id=chat_id, text=f"{file_name} fayli bo'yicha ma'lumotlar yuborildi.")
    except Exception as e:
...
async def start_info_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [[InlineKeyboardButton(file, callback_data=file)] for file in EXCEL_FILES]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Qaysi faylni tekshirishni xohlaysiz?", reply_markup=reply_markup)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(text=f"Tanlangan fayl: {selected_file}. Ma'lumotlarni tekshirish boshlandi...")
    await process_excel_file(selected_file, query.from_user.id, context)
